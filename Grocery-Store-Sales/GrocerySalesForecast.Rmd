...
``` {r,echo = FALSE}
sales<-read.csv(file = "C:\\Users\\phili\\Desktop\\Projects\\Forecasting\\Grocery-Store-Sales\\GroceryStoreSales.txt")
...
**Analysis: **  Upward trend is visible. In the Sales plot (blue line) Sales seem to increase exponentially or by a constant multiplicative factor, whereas in the log Sales plot (red line) log Sales seem to increase by a constant additive amount. 
...
Volatility seems to increase with the number of Sales, and economic contraction does not seem to affect grocery Sales. There is an outlier in March 2020 (observation 339), where grocery sales skyrocket due to the start of the COVID-19 pandemic.


**Causal Inference: ** The most obvious underlying cause of the peak in December Sales is Christmas holiday shopping - consumers buy more groceries to do Christmas cooking. The outlying sharp sales peak in March 2020 is due to the start of COVID-19 pandemic which led Americans to stockpile groceries. 
...
**Comments on trend and seasonal components: **
...
**Comments: ** The output of code below produces log the seasonal indices, so we would need to exponentiate them to obtain the seasonal indices. The final indices will give us the percentage change of Sales from month to month. 
...
**Save the residuals from the fit. **

```{r}
res1 <- resid(model1)
```

**(Form a normal quantile plot of these residuals**
...
**and plot their autocorrelations. **
...
**More Analysis on the residuals**
...
**Plotting and comparing seasonal indices**
...
**Analysis of lag residuals:** The residuals plot is now flat and less volatile, which means that the model is now closer to yielding white noise. The autocorrelation function plot now also indicates that most lags are between the blue lines. 
...
Investigating the presence of dynamic seasonality. Analyze the data spanning the years 1992 through 2007 and calculate the static seasonal estimates.
